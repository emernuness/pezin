...
export default function EditPackPage({ params }: EditPackPageProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (priceInCents < 1990) {
        toast.error("Preço deve ser no mínimo R$ 19,90");
        return;
      }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      toast.success(
        type === "preview" ? "Preview adicionado!" : "Arquivo adicionado!",
      );
  ...
  const handleDeleteFile = async (fileId: string) => {
    ...
    try {
      ...
      toast.error(
        axiosError.response?.data?.message || "Erro ao excluir arquivo",
      );
    } finally {
      setDeleting(null);
    }
  ...
  const handleDeletePreview = async (previewId: string) => {
    ...
    try {
      ...
      toast.error(
        axiosError.response?.data?.message || "Erro ao excluir preview",
      );
    } finally {
      setDeleting(null);
    }
          ...
          <div className="flex gap-2">
            {pack.status === "draft" && (
              <Button
                ...
                disabled={!canPublish}
                title={
                  !canPublish ? "Necessario: 1 preview e 3 arquivos" : undefined
                }
              >
                Publicar
              </Button>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Mínimo R$ 19,90 - Sem limite máximo
            </p>
          </div>
      ...
      {pack.status === "draft" && (
        <div className="mt-8 rounded-xl border border-amber-200 bg-amber-50 p-4">
          ...
          <ul className="mt-2 space-y-1 text-sm text-amber-700">
            <li
              className={
                pack.previews.length >= 1 ? "line-through opacity-50" : ""
              }
            >
            ...
            <li
              className={
                pack.files.length >= 3 ? "line-through opacity-50" : ""
              }
            >
